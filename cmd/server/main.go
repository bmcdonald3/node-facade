...
var rootCmd = &cobra.Command{
	Use:   "node-facade",
	Short: "",
	Long:  `node-facade - A Fabrica-generated OpenCHAMI service`,
	RunE:  runServer,
}

var serveCmd = &cobra.Command{
	Use:   "serve",
	Short: "Start the node-facade server",
	Long:  `Start the node-facade HTTP server with the configured options`,
	RunE:  runServer,
}
...
func init() {
	...
	// Global flags
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.node-facade.yaml)")
...
func initConfig() {
	...
	if cfgFile != "" {
		...
		viper.SetConfigName(".node-facade")
	}

	// Environment variables
	viper.SetEnvPrefix("NODE-FACADE")
	viper.AutomaticEnv()
...
func runServer(cmd *cobra.Command, args []string) error {
	log.Printf("Starting node-facade server...")
	...
	if eventConfig.EventTypePrefix == "" {
		eventConfig.EventTypePrefix = "node-facade.resource"
	}
...
func healthHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte(`{"status":"healthy","service":"node-facade"}`))
}
...
var versionCmd = &cobra.Command{
	...
	Long:  `Print the version number of node-facade`,
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Println("node-facade v1.0.0")
	},
}
